...
// v14.2 - Reintegrar stock de pedidos
const reintegrateOrderBtn=document.getElementById("reintegrateOrderBtn");
function orderStatusLabelV142(s){return({pending:"Pendiente",confirmed:"Confirmado",completed:"Confirmado",delivered:"Entregado",cancelled:"Cancelado",reintegrated:"Reintegrado"})[s]||"Pendiente"}
function renderOrderTimelineV142(order){const ev=Array.isArray(order.events)?order.events:[];if(!ev.length)return '<div class="order-timeline-empty-v142">Sin movimientos registrados.</div>';return `<div class="order-timeline-v142"><h4>Movimientos del pedido</h4>${ev.slice().reverse().map(e=>`<div class="order-timeline-item-v142"><span></span><div><strong>${e.message||e.type}</strong><small>${formatOrderDateV14(e.at)}</small></div></div>`).join("")}</div>`}
function renderOrderPreviewV142(order){if(!orderPreview)return;const items=(order.items||[]).map(i=>`<div class="order-preview-item"><strong>${i.name}</strong><span>${i.code||i.cardKey} · ${i.variant==="foil"?"Foil":"Normal"} x${i.qty}</span><b>$${Number(i.subtotal||0).toLocaleString("es-CL")} CLP</b></div>`).join("");const stock=order.stockReintegrated||order.status==="reintegrated"?"Stock reintegrado":order.stockDiscounted?"Stock descontado":"Stock sin descontar";orderPreview.innerHTML=`<div class="order-preview-head"><div><strong>Pedido #${order.id}</strong><small>Creado: ${formatOrderDateV14(order.createdAt)}</small></div><span class="order-status-pill ${orderStatusClassV14(order.status)}">${orderStatusLabelV142(order.status)}</span></div><div class="order-stock-state-v142">${stock}</div><div class="order-dates-v14"><span>Confirmado: ${formatOrderDateV14(order.confirmedAt||order.completedAt)}</span><span>Entregado: ${formatOrderDateV14(order.deliveredAt)}</span><span>Cancelado: ${formatOrderDateV14(order.cancelledAt)}</span><span>Reintegrado: ${formatOrderDateV14(order.reintegratedAt)}</span></div><div class="order-preview-list">${items}</div><div class="order-preview-total"><span>Total</span><strong>$${Number(order.total||0).toLocaleString("es-CL")} CLP</strong></div>${renderOrderTimelineV142(order)}`}
async function reintegrateOrderV142(){const id=cleanOrderId(orderIdInput?.value);if(!id||id==="00000"){showMessage("Ingresa un número de pedido válido.",true);return}if(!confirm(`¿Reintegrar al stock todas las cartas del pedido #${id}?`))return;await adminOrderRequestV14("reintegrate",id)}
renderOrderPreview=renderOrderPreviewV142;reintegrateOrderBtn?.addEventListener("click",reintegrateOrderV142);
